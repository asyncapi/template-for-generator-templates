...
- [.github/workflows/automerge-release-pr-bump.yml](.github/workflows/automerge-release-pr-bump.yml) workflow handless automated merging of a pull request created by the bot to bump the template version in the `package.json` file. The best approach is always to create a separate bot user that handles automation, but you can also use `github-actions` bot.
